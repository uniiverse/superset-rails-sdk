  ...
  class DatasetRestApiGetList
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::DatasetRestApiGetList` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::DatasetRestApiGetList`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'explore_url')
        self.explore_url = attributes[:'explore_url']
      end
    ...
    def hash
      [changed_by, changed_by_name, changed_by_url, changed_on_delta_humanized, changed_on_utc, database, datasource_type, default_endpoint, description, explore_url, extra, id, kind, owners, schema, sql, table_name].hash
    end
