  ...
  class ChartDataExtras
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::ChartDataExtras` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::ChartDataExtras`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      relative_end_validator = EnumAttributeValidator.new('Object', ['today', 'now'])
      return false unless relative_end_validator.valid?(@relative_end)
      relative_start_validator = EnumAttributeValidator.new('Object', ['today', 'now'])
      return false unless relative_start_validator.valid?(@relative_start)
      time_grain_sqla_validator = EnumAttributeValidator.new('Object', ['PT1S', 'PT5S', 'PT30S', 'PT1M', 'PT5M', 'PT10M', 'PT15M', 'PT30M', 'PT1H', 'PT6H', 'P1D', 'P1W', 'P1M', 'P3M', 'P1Y', '1969-12-28T00:00:00Z/P1W', '1969-12-29T00:00:00Z/P1W', 'P1W/1970-01-03T00:00:00Z', 'P1W/1970-01-04T00:00:00Z'])
    ...
    def relative_end=(relative_end)
      validator = EnumAttributeValidator.new('Object', ['today', 'now'])
