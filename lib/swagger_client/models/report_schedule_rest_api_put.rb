  ...
  class ReportScheduleRestApiPut
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::ReportScheduleRestApiPut` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::ReportScheduleRestApiPut`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [active, chart, context_markdown, creation_method, crontab, dashboard, database, description, force_screenshot, grace_period, log_retention, name, owners, recipients, report_format, sql, timezone, type, validator_config_json, validator_type, working_timeout].hash
    end
