  ...
  class DatasetRestApiPut
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::DatasetRestApiPut` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::DatasetRestApiPut`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [cache_timeout, columns, database_id, default_endpoint, description, external_url, extra, fetch_values_predicate, filter_select_enabled, is_managed_externally, is_sqllab_view, main_dttm_col, metrics, offset, owners, schema, sql, table_name, template_params].hash
    end
