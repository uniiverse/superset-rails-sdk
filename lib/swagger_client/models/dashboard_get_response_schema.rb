  ...
  class DashboardGetResponseSchema
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::DashboardGetResponseSchema` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::DashboardGetResponseSchema`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [certification_details, certified_by, changed_by, changed_by_name, changed_by_url, changed_on, changed_on_delta_humanized, charts, css, dashboard_title, id, is_managed_externally, json_metadata, owners, position_json, published, roles, slug, thumbnail_url, url].hash
    end
